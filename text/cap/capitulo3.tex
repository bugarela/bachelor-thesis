...
Seção \ref{sec:condicoes}, deseja-se separá-los em condiçoes \ttc\ e ações sobre
...
A tradução de uma especificação é feita através das regras na Figura
\ref{fig:t-specification}. As definições são traduzidas pela regra (\texttt{DEF}) que, a partir das duplas
...
Elixir, a partir do operador \texttt{@}, assim como uma função para acessá-la de
...
A regra (\texttt{MOD}) é responsável pelo início da tradução, recebendo a
especificação completa e traduzindo cada elemento com o conjunto de regras
devido. A partir dela, um módulo Elixir é obtido, assim como a invocação de sua
...
\texttt{@}. Caso haja uma indicação de módulo, é necessário traduzí-la para a
...
traria segurança de que, idependentemente de quais dos estados iniciais definidos o
