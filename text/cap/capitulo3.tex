...
para duas definições com suas próprias condições e ações, de forma que uma das
definições tenha a condição idêntica a condição do \IF e a outra, sua negação.
Para preservar a semelhança com a especificação, contudo, optou-se por utilizar
a própria estrutura \texttt{if} da linguagem destino. 

Quando as definições dos blocos \THEN e \textsc{else} não possuem condições de ativação,
é possível aplicar a regra (\texttt{IF-DIRECT}), que não gera nenhuma condição,
uma vez que ambas as ações podem ser ativadas, independentemente da avaliação da
condicional. Essa regra lida com o caso da regra (\texttt{IF-COND}) onde não há
condições de ativação, porém o resultado para as duas regras é equivalente. A
regra (\texttt{IF-DIRECT}) tem como função evitar que uma condição de ativação verbosa
que sempre avalia para verdadeiro seja gerada, melhorando a legibilidade e
performance.

Em casos onde alguma das definições apresenta uma condição de avaliação, a regra
(\texttt{IF-COND}) precisa ser aplicada, carregando a condição para fora da
estrutura \texttt{if}, já que ela representa apenas uma transformação de estado
dependente de uma condicional - uma vez que a ação contendo essa estrutura for
invocada, ela precisa retornar um novo estado. Por isso, é necessário avaliar as
condições antes e não durante a ação. Para essa ação ser ativável, é necessário
...
pelo OU: Uma identificação da definição (gerada por uma função \texttt{show} de
conversão da estrutura para \texttt{string}); A redução das condições
encontradas na definição; e a redução das ações.
