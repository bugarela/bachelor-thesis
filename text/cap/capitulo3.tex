...
\begin{figure}[h]
  ...
  \fbox{$\begin{array}{l} \begin{array}{lrll}
    ...
    \end{array} \\ \\
    \begin{array}{lrcl}
  \text{Ação} & a & ::= & \texttt{\%\{ i : x \}} \,\mid\, \texttt{Map.merge(a, a)} \smallskip\\
  \text{Condição} & c & ::= & \texttt{not c} \,\mid\, \texttt{c and c} \,\mid\,
                              \texttt{c or c}\smallskip\\
  \text{Conjunto} & s & ::= & \texttt{MapSet.new([x])} \,\mid\, \texttt{MapSet.union($s_a, s_b$)}\smallskip\\
  \text{Registro} & r & ::= & \texttt{\%\{ k: x \}} \,\mid\, \texttt{Map.put(i,
                              k, x)}\smallskip\\
  \end{array} \end{array}$}
  \caption{Notação para a linguagem Elixir}
