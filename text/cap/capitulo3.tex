...
podem ser interpretadas como condições de ativação. A ação só pode ser ativada
...
Um par de estados pode atender mais de uma ação especificada em \TLAA.
Quando isso acontece, o TLC avalia todos os pares estados possíveis distintos
e que atendem as condições da ação, verificando os caminhos que surgem de outras
ações a partir de cada um deles. No programa
gerado, contudo, executar uma ação significa fazer uma alteração de estado, e
múltiplas alterações de estado não são muito interessantes para o propósito do
...
partir de um estado. Para garantir que seja possível tomar tais decisões, é
necessário que a especificação defina condições de ativação mutualmente
exclusivas para ações disjuntas - isto é, sempre que há uma disjunção de ações,
todas as condições de ativação dessas ações devem ser falsas exceto pela daquela
